  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 via-purple-100 to-pink-50 dark:from-gray-900 dark:to-gray-800 transition-all">
      <div className="w-full max-w-2xl p-6 rounded-xl bg-white dark:bg-gray-900 shadow-xl">
        <h1 className="text-3xl font-bold text-center text-gray-800 dark:text-white mb-6">🚀 Welcome to <span className="text-blue-600">CodeQuest</span></h1>
        ...
        {history.length > 0 && (
          <div className="mt-8">
            ...
            <ul className="space-y-3 max-h-64 overflow-auto pr-1">
              {history.map((session) => (
                <li key={session.id} className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg flex justify-between items-center">
                  <div>
                    <p className="text-sm text-gray-800 dark:text-white">
                      <span className="font-medium text-blue-600">{session.username}</span> joined room <span className="font-mono">{session.roomId}</span>
                    </p>
